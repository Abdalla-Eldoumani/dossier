...
export function DeletePagesOperation() {
  ...
  const onRun = async () => {
    ...
    try {
      ...
      const out = await worker.deletePages(staged.bytes, indices);
      const name = staged.name.replace(/\.pdf$/i, "") + ".trimmed.pdf";
      await downloadBytes(out.bytes, name);
      toast.success(`Removed ${indices.length} page${indices.length === 1 ? "" : "s"}.`);
    } catch (err) {
      if (err instanceof PageIndicesError) {
        toast.error("Check the page list.", { description: err.message });
      } else {
        toast.error("Could not delete pages.", {
  ...
  return (
    ...
    >
      ...
      <p className="text-[13px] text-[var(--color-ink-3)] max-w-prose">
        One-based page numbers. Comma-separated, ranges with a hyphen. The remaining
        pages keep their original order.
      </p>
