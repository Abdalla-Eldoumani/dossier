...
export function DuplicatePagesOperation() {
  ...
  const onRun = async () => {
    ...
    try {
      ...
      const out = await worker.duplicatePages(staged.bytes, indices);
      const name = staged.name.replace(/\.pdf$/i, "") + ".duplicated.pdf";
      await downloadBytes(out.bytes, name);
      toast.success(`Duplicated ${indices.length} page${indices.length === 1 ? "" : "s"}.`);
    } catch (err) {
      if (err instanceof PageIndicesError) {
        toast.error("Check the page list.", { description: err.message });
      } else {
        toast.error("Could not duplicate pages.", {
  ...
  return (
    ...
    >
      ...
      <p className="text-[13px] text-[var(--color-ink-3)] max-w-prose">
        Each duplicated page is appended after its original. Use{" "}
        <code style={{ fontFamily: "var(--font-mono)" }}>Reorder</code> afterwards if you
