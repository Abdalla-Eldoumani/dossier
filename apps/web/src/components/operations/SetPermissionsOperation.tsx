...
export function SetPermissionsOperation() {
  ...
  const onRun = async () => {
    ...
    if (!ownerPassword) {
      toast.error("Owner password required.", {
        description: "pdf-lib can’t rewrite the permission bitmap without it.",
      });
      return;
    }
    setBusy(true);
    try {
      ...
      const out = await worker.setPermissions(staged.bytes, { ownerPassword, permissions });
      const name = staged.name.replace(/\.pdf$/i, "") + ".permissions.pdf";
  ...
  return (
    ...
    >
      ...
      <p className="text-[13px] text-[var(--color-ink-3)] max-w-prose">
        Same `PdfSecurity` provider as encrypt — needs the owner password because
        pdf-lib can’t rewrite the permission bitmap on its own.
      </p>
      ...
      <button
        ...
        className={cn(
          ...
          busy || !ownerPassword ? "opacity-60 cursor-not-allowed" : "hover:opacity-90",
        )}
      >
        <ShieldCheck size={16} strokeWidth={1.5} aria-hidden />
        {busy ? "Updating…" : "Set permissions"}
      </button>
    </form>
  );
}
